...
app.post('/api/news', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO news (title, description, image_url, author_id) VALUES ($1, $2, $3, $4) RETURNING *',
            [title, description, image_url, author_id]
        );
...
app.get('/api/messages/:chatId', async (req, res) => {
    try {
        const result = await pool.query(`SELECT m.*, u.nickname as sender_name, u.rank as sender_rank, u.avatar_url as sender_avatar, cm.role as sender_role FROM messages m JOIN users u ON m.sender_id = u.id JOIN chat_members cm ON m.chat_id = cm.chat_id AND m.sender_id = cm.user_id WHERE m.chat_id = $1 ORDER BY m.created_at ASC`, [req.params.chatId]);
        res.json(result.rows);
    } catch (err) { res.status(500).json({ error: 'Ошибка' }); }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('send_msg', async (data) => {
        try {
            const insRes = await pool.query('INSERT INTO messages (chat_id, sender_id, text) VALUES ($1, $2, $3) RETURNING id', [data.chatId, data.senderId, data.text]);
            const msgFull = await pool.query(`SELECT m.*, u.nickname as sender_name, u.rank as sender_rank, u.avatar_url as sender_avatar, cm.role as sender_role FROM messages m JOIN users u ON m.sender_id = u.id JOIN chat_members cm ON m.chat_id = cm.chat_id AND m.sender_id = cm.user_id WHERE m.id = $1`, [insRes.rows[0].id]);
